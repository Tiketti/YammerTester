function YCtrl($scope, $http) {
	...
	$scope.yammerPostEnabled = false;

	$scope.readMenus = function() {
		console.dir('read menus');

		$http.get('api/menus/get')
			.success(function(data) {
				...
				//TODO: also should be logged in
				$scope.yammerPostEnabled = $scope.menus.length > 0;
	...
	// for testing purposes, not to be used in production
	// fetch menus and have user authenticate to Yammer
	// then post in user's name
	$scope.postMenusToYammer = function() {
